...
- `applyLibjxlPredictor` — all 14 spec / libjxl Modular predictors (`Zero`, `Left`, `Top`, `Average0`, `Select`, `ClampedGradient`, `Weighted`, `TopRight`, `TopLeft`, `LeftLeft`, `Average1..4`) directly from the raw 0..13 index. Source of truth for byte-exact pixel reconstruction; predictor 6 (Weighted) takes a caller-supplied `wpResult` and returns 0 until the WP state machine lands.
- `decodeModularChannel` (§C.7) — streaming per-pixel decode driver for one Modular channel. Iterates row-major; for each pixel computes neighbours + properties → walks the tree → reads a token via `TokenStreamReader` at `context = leaf.leafId` → `ZigZag.unpack` × multiplier → adds predictor output + leaf offset.
...
The codestream reader chain now walks **thirteen spec layers deep** into a real cjxl-emitted Modular lossless file and runs all 1024 pixels of a 32×32 channel through the per-pixel pipeline (rANS state init + 1024 token reads + 1024 tree walks + 1024 predictor/offset/multiplier applications). Layers: signature → SizeHeader → ImageMetadata → FrameHeader → TOC → DequantMatrices DC flag → Modular `has_tree` → tree-section EntropySectionHeader → per-cluster Huffman tables → MA-tree token stream → typed `ModularTree` → post-tree pixel-data EntropySectionHeader → byte-aligned **`GroupHeader`** → **per-pixel `decodeModularChannel`**. The decoded pixels aren't yet byte-identical to djxl: predictor 6 (Weighted) and property 15 (kWPProp) need a stateful WP machine, and Modular Transforms (RCT inverse, Squeeze inverse) need to run on the reconstructed channel arrays. Both are the next two roadmap items.
...
swift test  -c release           # 224 tests (foundation + headers + entropy primitives + frame header + TOC + entropy section bodies + streaming rANS + Modular tree + tree walk + properties + GroupHeader + per-pixel decoder + cross-validation 13-layers-deep), ~50 ms
