...
swift test  -c release           # 227 tests (foundation + headers + entropy primitives + frame header + TOC + entropy section bodies + streaming rANS + Modular tree + tree walk + properties + GroupHeader + per-pixel decoder + WeightedPredictor + cross-validation 13-layers-deep), ~50 ms
