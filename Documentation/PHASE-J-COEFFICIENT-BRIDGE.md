...
| 8   | `jxl transcode --mode reverse` wiring + tests | ⏳ — `JXLToJPEGAdapter.reconstruct(...)` is shipping. The CLI wire-up needs (a) JXL container parser to find the `jbrd` box; (b) JXL frame decode to recover coefficient planes; (c) `JPEGCoefficientPlanes` ↔ `JXLCoefficientPlanes` shape detection. Most pieces are in place from the forward direction. |
