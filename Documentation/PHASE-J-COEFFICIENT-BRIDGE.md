...
| 4   | Lift the 4:4:4-only restriction in the adapter (4:2:0 / 4:2:2 chroma subsampling support) | ✅ structural (v0.12.0ft) — adapter accepts any of `{H1V1, H2V1, H1V2, H2V2}` Y sampling with H1V1 chroma; `JXLCoefficientPlanes` carries per-channel block dims; `chroma_subsampling` FrameHeader field computed from JPEG factors; DC sub-image + AC token writers iterate per-channel grids. 4:4:4 pixel parity preserved (max=2). **4:2:0 path is decodable but not yet pixel-equivalent** (max=31, mean=18.5 vs reference — first byte diff at TOC entry-size, suggesting ~18 extra bytes of section content vs cjxl reference). Next bite: localise the residual section-content diff (likely codebook overhead and/or AC-token-count differences for chroma blocks). |
...
**Next concrete bite when picking this back up:** lift the 4:4:4-only restriction (step 4 — chroma subsampling). The forward bridge is **shipping pixel-equivalent** as of v0.12.0fr; the natural next step is supporting subsampled chroma so the bridge accepts sips-generated JPEGs (which default to 4:2:0). The adapter currently throws `.nonUniformSampling` for non-4:4:4 inputs at `JPEGCoefficientImage.toJXLCoefficientPlanes()`. The hard work is propagating per-channel sampling factors through the channel dimensions in the DC sub-image + AC group iteration so each chroma channel emits half-resolution coefficients in the appropriate cells.
