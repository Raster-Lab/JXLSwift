...
**Next concrete bite when picking this back up:** the forward bridge now ships **pixel-equivalent for 4:4:4 multi-block and 4:2:0 within chroma-upsampling tolerance**. The remaining direction-forward work that's NOT done: (1) tighten the 4:2:0 chroma-upsampling residual (max=9 currently — figure out whether libjxl's chroma upsample filter is achievable in our reference path); (2) extend the bridge to handle the 4:2:2 cases that the structural lift accepts but no end-to-end test exercises yet; (3) handle non-uniform per-block QF / non-DCT8×8 AC strategies once the spec coverage matures. The major next phase is **reverse direction** (Phase J step 5+): pure-Swift Brotli decoder (4-8 sessions), `jbrd` box parser, and reverse-bridge implementation that takes a JXL frame produced by our forward bridge plus its `jbrd` box and reconstructs the source JPEG byte-for-byte. The `jbrd` box specifies Huffman tables, marker order, and byte-stuffing positions — without it, JPEG → JXL → JPEG re-encodes the pixels lossily. For byte-identical reverse, the `jbrd` data must be preserved on the forward pass (we don't yet emit a `jbrd` box — that's part of step 5).
