...
| 5   | Pure-Swift Brotli decoder (unblocks `jbrd` + compressed ICC) | 🚧 scaffold shipped v0.12.0fz: `Sources/JXLSwift/Brotli/` directory with `BrotliErrors`, `BrotliBitReader`, `BrotliPrefixCode` (simple + complex format, canonical Huffman decode), `BrotliMetaBlock` (stream header WBITS verified vs `brotli --lgwin=N`; meta-block header for ISLAST/MNIBBLES/MLEN/ISUNCOMPRESSED). 14 unit tests. Remaining: literal/insert+copy/distance alphabets (RFC 7932 §7), context modeling, static dictionary (~120KB + transforms, §8), LZ77 reconstruction loop. |
| 6   | `jbrd` box parser | 🚧 scaffold shipped v0.12.0g0: `JPEG/JBRDBox.swift` with `JBRDBox` struct (+ JBRDQuantTable/HuffmanCode/Component/ScanInfo) + `JBRDError` taxonomy + `JBRDBoxReader.read`/`JBRDBoxWriter.write` entry points stubbed. Remaining: the Bundle field-by-field walk implementing libjxl's `JPEGData::VisitFields`. |
| 7   | Reverse bridge implementation | ✅ **coefficient-identical** shipped v0.12.0g1–g5: `JXLCoefficientPlanes.inverseJXLBridgeRemap` + `inverseJPEGBridgeDC` + `toJPEGCoefficientImage` (transpose-back); `JPEGBitWriter` + `JPEGBlockEncoder` + `JPEGScanEncoder` + `JPEGContainerWriter`; `JXLToJPEGAdapter.reconstructMinimal(...)` capstone API. 17 round-trip tests + 1 end-to-end test (`testEndToEnd_ForwardThenReverseBridge_CoefficientsMatch`). Byte-identical reconstruction (`reconstruct(...)` with jbrd input) is `notImplemented` pending steps 5 + 6. |
| 8   | `jxl transcode --mode reverse` wiring + tests | ⏳ — needs steps 5 + 6 complete for byte-identical output; can be wired now against `reconstructMinimal` for coefficient-identical mode. |

**Next concrete bite when picking this back up:** the **reverse direction** (Phase J step 5) now ships at the **coefficient-identicality** level — see v0.12.0g5. Real JPEG → forward bridge → JXL planes → reverse bridge (`JXLToJPEGAdapter.reconstructMinimal`) → rebuilt JPEG, and decoding the rebuilt JPEG produces matching coefficient values (verified by `testEndToEnd_ForwardThenReverseBridge_CoefficientsMatch`). The pieces in place: `JXLCoefficientPlanes.inverseJXLBridgeRemap` + `inverseJPEGBridgeDC` + `toJPEGCoefficientImage` (transpose-back); `JPEGBitWriter` + `JPEGBlockEncoder` + `JPEGScanEncoder` + `JPEGContainerWriter`. **Byte-identical** reconstruction (where the rebuilt JPEG matches the source byte-for-byte) needs three more pieces: (a) the Brotli decoder (scaffolded at v0.12.0fz — `Sources/JXLSwift/Brotli/` has the bit reader + prefix code reader + meta-block header reader + 14 tests; alphabets / context modeling / static dictionary / LZ77 are still to be filled, ~4-8 sessions); (b) `JBRDBoxReader.read` field-by-field walk (struct + entry points scaffolded at v0.12.0g0, walk pending); (c) the `JXLToJPEGAdapter.reconstruct(coefficients:jbrd:colorTransform:)` integration that uses jbrd's `markerOrder` to replay the source JPEG's marker layout exactly. Until those three land, callers wanting reverse-bridge output should use `reconstructMinimal(...)` which produces a structurally valid JPEG (decoded by djpeg cleanly, matches coefficient values) but won't match the source byte-for-byte.
