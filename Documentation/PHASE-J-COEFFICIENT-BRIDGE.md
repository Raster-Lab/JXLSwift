...
| 4   | Lift the 4:4:4-only restriction in the adapter (4:2:0 / 4:2:2 chroma subsampling support) | ✅ structural (v0.12.0ft) — adapter accepts any of `{H1V1, H2V1, H1V2, H2V2}` Y sampling with H1V1 chroma; `JXLCoefficientPlanes` carries per-channel block dims; `chroma_subsampling` FrameHeader field computed from JPEG factors; DC sub-image + AC token writers iterate per-channel grids. 4:4:4 single-block pixel parity preserved (max=2). **Residual recharacterised at v0.12.0fw: the remaining pixel diff is a MULTI-BLOCK bug, not chroma-specific.** A 4:4:4 16×16 control test (`…_RealJPEG444_16x16_DjxlAccepts`) emits `max=74, mean=21.83` — worse than the 4:2:0 case (`max=31, mean=18.5`). The 4:4:4 single-block baseline only passed because the test fixture is 8×8 (one block per channel). Next bite: localise the multi-block AC pipeline divergence — Y blocks beyond `(0, 0)` decode to wrong coefficients in djxl. Candidates: AC token offset tracking, nnz prediction plane for non-first blocks, or per-block context lookup. |
...
**Next concrete bite when picking this back up:** localise the **multi-block AC pipeline bug** uncovered at v0.12.0fw. The 4:4:4 16×16 control test `testJXLEncoder_FromJPEGCoefficients_RealJPEG444_16x16_DjxlAccepts` is the pin-down — it asserts `max ≤ 64` and currently reports `max=74`. The same 16×16 gradient through 4:2:0 reports `max=31` (smaller because chroma planes have only 1 block each — only Y multi-block stress applies). The 4:4:4 8×8 baseline still reports `max=2` (passes because single-block doesn't exercise the bug). What the chroma-subsampling work at v0.12.0ft accomplished correctly: per-channel block grids in `JXLCoefficientPlanes`, `chroma_subsampling` FrameHeader, AC token chroma gating, per-channel DC sub-image dims. What's still wrong: something in the AC pipeline produces wrong coefficient values for Y blocks beyond `(0, 0)`. Investigation candidates: (a) AC token offset tracking across blocks (libjxl `enc_entropy_coder.cc:182-195` increments `offset[c]` per emitted block); (b) the nnz prediction plane lookup for non-first blocks (verify `predNnz` from top + left matches libjxl's `PredictFromTopAndLeft`); (c) per-block context lookup (verify `BlockCtxMap.context(dcIdx, qf, ord, c)` returns the same value as libjxl for blocks where qdc/qf vary). Use `testDiagnostic_Dump44416BridgeBytes` to dump our bridge bytes and compare to `/tmp/cjxl-ref-444-16.codestream`.
