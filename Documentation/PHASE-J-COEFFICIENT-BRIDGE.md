...
| 3.7 — real-content fixture + JPEGDecoder pixel parity | non-zero DC + AC fixture → `djxl(bridgeBytes)` pixels match `JPEGDecoder.decode(jpgBytes)` pixels | ⏳ in progress (v0.12.0fo). djxl decodes a quality-75 4:4:4 JPEG without saturating after the DC scale-inversion fix (max diff 139, mean 34 vs original max 209, mean 114, all-0xFF). Remaining gap: at least one more scaling factor mismatch in the AC dequant / inverse YCbCr / IDCT pipeline — characterized but not yet localised. |
