...
| 5   | Pure-Swift Brotli decoder (unblocks `jbrd` + compressed ICC) | 🎉 **compressed body shipping** v0.12.0fz → ga → gb → gk → gl → gm → gn. Full uncompressed + compressed Brotli streams for the common shape (NBLTYPES=1, NTREES=1, no static dictionary references) decode end-to-end. Components: `BrotliErrors`, `BrotliBitReader` (incl. `readVarLenU8`), `BrotliPrefixCode` (simple + complex format, source-order length assignment for NSYM=3+treeSelect=1), `BrotliMetaBlock` (stream header + meta-block header), `BrotliCompressedMetaBlock` (NBLTYPES + NPOSTFIX + NDIRECT + context modes + NTREES), `BrotliInsertCopy` (704-entry IC LUT + per-command decoder), `BrotliDistance` (LUT builder + ring buffer + short-code resolver + readDistance), `BrotliDecoder` (top-level shell + uncompressed path + LZ77 reconstruction loop). 27 unit tests pass on real cjxl-emitted streams. Remaining: static dictionary (RFC 7932 §8, ~120KB embedded table + 121 transforms), NBLTYPES > 1 (block-length walker), NTREES > 1 (context map decoder). |
