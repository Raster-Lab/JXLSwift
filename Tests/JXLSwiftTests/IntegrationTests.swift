...
extension FoundationTests {
    ...
    /// DCT32×32 encoder DSP foundation. `dcFromLowestFrequencies32x32`
    /// must exactly invert the decoder's `dct32x32`, so the encoder
    /// can split a DCT32×32 block's 16 low-frequency coefficients
    /// into the 16 DC-plane cell values.
    func testLowestFrequenciesFromDC_DCT32x32_Inverse() throws {
        let cases: [[Float]] = [
            [Float](repeating: 100, count: 16),         // flat
            (0..<16).map { Float($0) * 3 - 20 },
            (0..<16).map { Float(($0 * 7) % 13) - 6 },
            [Float](repeating: 0, count: 16),
        ]
        for dc in cases {
            let llf = LowestFrequenciesFromDC.dct32x32(dc: dc)
            let back = LowestFrequenciesFromDC
                .dcFromLowestFrequencies32x32(llf: llf)
            for i in 0..<16 {
                XCTAssertEqual(back[i], dc[i], accuracy: 1e-2,
                    "DCT32x32 LLF inverse mismatch at \(i): "
                    + "dc=\(dc) → \(back)")
            }
        }
    }

    /// DCT32×32 encoder DSP foundation. `forwardDCT32Block`
    /// (forward DCT32 + LLF→DC split + AC quantise) round-trips a
    /// smooth 32×32 patch through the decoder's DCT32 reconstruction
    /// (`dct32x32` LLF + dequant + transpose + IDCT32) within a
    /// bounded error.
    func testVarDCTEncoder_ForwardDCT32Block_RoundTrip() throws {
        var patch = [Float](repeating: 0, count: 1024)
        for y in 0..<32 {
            for x in 0..<32 {
                patch[y * 32 + x] = 0.12 + Float(x + y) * 0.006
            }
        }
        let qw = try QuantWeights.getQuantWeights(
            rows: 32, cols: 32, bands: DefaultQuantBands.dct32x32)
        let yWeights = Array(qw[1024..<2048])         // Y channel
        let globalScale: UInt32 = 5111
        let qf: Int32 = 5
        let acScale = Float(globalScale) / 65536.0
        let (dc, ac) = VarDCTEncoder.forwardDCT32Block(
            patch: patch, quantWeights: yWeights,
            scale: acScale, qf: qf)
        XCTAssertEqual(dc.count, 16)
        XCTAssertEqual(ac.count, 1024)
        // Decoder reconstruction: LLF from DC, dequant AC, IDCT32.
        var coef = [Float](repeating: 0, count: 1024)
        let llf = LowestFrequenciesFromDC.dct32x32(dc: dc)
        for r in 0..<4 {
            for c in 0..<4 { coef[r * 32 + c] = llf[r * 4 + c] }
        }
        let invQuantAC = 65536.0 / Float(globalScale) / Float(qf)
        for np in 0..<1024 {
            let r = np / 32, c = np % 32
        ...
        JXLDecoder.transposeSquareInPlace(&coef, size: 32)
        AccelerateDCT.idct2D(&coef, size: 32)
        var meanErr: Float = 0, maxErr: Float = 0
        for i in 0..<1024 {
            let e = abs(coef[i] - patch[i])
            meanErr += e
        ...
        meanErr /= 1024
        XCTAssertLessThan(meanErr, 0.03,
            "DCT32 block round-trip mean error \(meanErr)")
        XCTAssertLessThan(maxErr, 0.12,
            "DCT32 block round-trip max error \(maxErr)")
    }
    ...
    /// DCT16×8 / DCT8×16 encoder DSP foundation.
    /// `dcFromLowestFrequenciesOrd4Pair` must exactly invert the
    /// decoder's `ord4Pair`, so the encoder can split a `ord 4`
    /// block's 2 low-frequency coefficients into the 2 covered
    /// cells' DC-plane values.
    func testLowestFrequenciesFromDC_Ord4Pair_Inverse() throws {
        let cases: [[Float]] = [
            [100, 100], [50, 80], [-5, 17], [0, 0], [255, -128],
        ]
        for dc in cases {
            let llf = LowestFrequenciesFromDC.ord4Pair(dc: dc)
            let back = LowestFrequenciesFromDC
                .dcFromLowestFrequenciesOrd4Pair(llf: llf)
            for i in 0..<2 {
                XCTAssertEqual(back[i], dc[i], accuracy: 1e-3,
                    "ord4Pair LLF inverse mismatch at \(i): "
                    + "dc=\(dc) → \(back)")
            }
        }
    }

    /// DCT8×16 encoder DSP foundation. `forwardDCT8x16Block`
    /// (forward asymmetric DCT + LLF→DC split + AC quantise)
    /// round-trips an 8-row × 16-col patch through the decoder's
    /// DCT8×16 reconstruction (`ord4Pair` LLF + dequant +
    /// `idct2D(rows:8, cols:16)`) within a bounded error.
    func testVarDCTEncoder_ForwardDCT8x16Block_RoundTrip() throws {
        var patch = [Float](repeating: 0, count: 128)
        for r in 0..<8 {
            for c in 0..<16 {
                patch[r * 16 + c] = 0.12 + Float(r + c) * 0.008
            }
        }
        let qw = try QuantWeights.getQuantWeights(
            rows: 8, cols: 16, bands: DefaultQuantBands.dct8x16)
        let yWeights = Array(qw[128..<256])        // Y channel
        let globalScale: UInt32 = 5111
        let qf: Int32 = 5
        let acScale = Float(globalScale) / 65536.0
        let (dc, ac) = VarDCTEncoder.forwardDCT8x16Block(
            patch: patch, quantWeights: yWeights,
            scale: acScale, qf: qf)
        XCTAssertEqual(dc.count, 2)
        XCTAssertEqual(ac[0], 0)
        XCTAssertEqual(ac[1], 0)
        // Decoder reconstruction.
        var coef = [Float](repeating: 0, count: 128)
        let llf = LowestFrequenciesFromDC.ord4Pair(dc: dc)
        coef[0] = llf[0]; coef[1] = llf[1]
        let invQuantAC = 65536.0 / Float(globalScale) / Float(qf)
        for np in 2..<128 {
        ...
        AccelerateDCT.idct2D(&coef, rows: 8, cols: 16)
        // DCT8×16: pixel = coef layout directly.
        var meanErr: Float = 0, maxErr: Float = 0
        for i in 0..<128 {
            let e = abs(coef[i] - patch[i])
            meanErr += e
        ...
        meanErr /= 128
        XCTAssertLessThan(meanErr, 0.03,
            "DCT8x16 block round-trip mean error \(meanErr)")
        XCTAssertLessThan(maxErr, 0.10,
            "DCT8x16 block round-trip max error \(maxErr)")
    }

    /// DCT16×8 encoder DSP foundation. `forwardDCT16x8Block`
    /// transposes the 16h × 8w patch into the 8-row × 16-col coef
    /// layout (the decoder transposes back after IDCT); the
    /// round-trip via the decoder's primitives stays within bound.
    func testVarDCTEncoder_ForwardDCT16x8Block_RoundTrip() throws {
        var patch = [Float](repeating: 0, count: 128)
        for r in 0..<16 {
            for c in 0..<8 {
                patch[r * 8 + c] = 0.12 + Float(r + c) * 0.008
            }
        }
        let qw = try QuantWeights.getQuantWeights(
            rows: 8, cols: 16, bands: DefaultQuantBands.dct8x16)
        ...
        let qf: Int32 = 5
        let acScale = Float(globalScale) / 65536.0
        let (dc, ac) = VarDCTEncoder.forwardDCT16x8Block(
            patch: patch, quantWeights: yWeights,
            scale: acScale, qf: qf)
        // Decoder reconstruction.
        var coef = [Float](repeating: 0, count: 128)
        ...
        AccelerateDCT.idct2D(&coef, rows: 8, cols: 16)
        // DCT16×8: pixel layout is the transpose of coef layout —
        // `pixel[r][c] = coef[c * 16 + r]` for r in 0..16, c in 0..8.
        var meanErr: Float = 0, maxErr: Float = 0
        for r in 0..<16 {
            for c in 0..<8 {
                let e = abs(coef[c * 16 + r] - patch[r * 8 + c])
                meanErr += e
                maxErr = max(maxErr, e)
            }
        }
        meanErr /= 128
        XCTAssertLessThan(meanErr, 0.03,
            "DCT16x8 block round-trip mean error \(meanErr)")
        XCTAssertLessThan(maxErr, 0.10,
            "DCT16x8 block round-trip max error \(maxErr)")
    }

    /// DCT32×16 / DCT16×32 encoder DSP foundation.
    /// `dcFromLowestFrequenciesOrd6Block` must exactly invert the
    ...
    func testLowestFrequenciesFromDC_Ord6Block_Inverse() throws {
        let cases: [[Float]] = [
            ...
            (0..<8).map { Float($0) * 5 - 15 },
            (0..<8).map { Float(($0 * 7) % 13) - 6 },
            [Float](repeating: 0, count: 8),
        ]
        for dc in cases {
            let llf = LowestFrequenciesFromDC.ord6Block(dc: dc)
            let back = LowestFrequenciesFromDC
                .dcFromLowestFrequenciesOrd6Block(llf: llf)
            for i in 0..<8 {
                XCTAssertEqual(back[i], dc[i], accuracy: 1e-2,
                    "ord6Block LLF inverse mismatch at \(i): "
                    + "dc=\(dc) → \(back)")
            }
        }
    }
    ...
    /// block's 64 LLF coefficients into the 64 DC-plane cell values.
    func testLowestFrequenciesFromDC_DCT64x64_Inverse() throws {
        let cases: [[Float]] = [
            [Float](repeating: 100, count: 64),
            (0..<64).map { Float($0) * 2 - 50 },
            (0..<64).map { Float(($0 * 7) % 23) - 10 },
            [Float](repeating: 0, count: 64),
        ]
        for dc in cases {
            let llf = LowestFrequenciesFromDC.dct64x64(dc: dc)
            let back = LowestFrequenciesFromDC
                .dcFromLowestFrequencies64x64(llf: llf)
            for i in 0..<64 {
                XCTAssertEqual(back[i], dc[i], accuracy: 5e-2,
                    "DCT64x64 LLF inverse mismatch at \(i)")
            }
        }
    }

    /// DCT64×64 encoder DSP foundation. `forwardDCT64x64Block`
    /// (forward DCT64 + LLF→DC split + AC quantise) round-trips a
    /// smooth 64×64 patch through the decoder's DCT64 reconstruction
    /// (`dct64x64` LLF + dequant + transpose + IDCT64) within a
    /// bounded error.
    func testVarDCTEncoder_ForwardDCT64x64Block_RoundTrip() throws {
        var patch = [Float](repeating: 0, count: 4096)
        for y in 0..<64 {
            for x in 0..<64 {
                patch[y * 64 + x] = 0.12 + Float(x + y) * 0.003
            }
        }
        let qw = try QuantWeights.getQuantWeights(
            rows: 64, cols: 64, bands: DefaultQuantBands.dct64x64)
        let yWeights = Array(qw[4096..<8192])
        let globalScale: UInt32 = 5111
        let qf: Int32 = 5
        let acScale = Float(globalScale) / 65536.0
        let (dc, ac) = VarDCTEncoder.forwardDCT64x64Block(
            patch: patch, quantWeights: yWeights,
            scale: acScale, qf: qf)
        XCTAssertEqual(dc.count, 64)
        XCTAssertEqual(ac.count, 4096)
        // Decoder reconstruction.
        var coef = [Float](repeating: 0, count: 4096)
        let llf = LowestFrequenciesFromDC.dct64x64(dc: dc)
        for r in 0..<8 {
            for c in 0..<8 { coef[r * 64 + c] = llf[r * 8 + c] }
        }
        ...
        JXLDecoder.transposeSquareInPlace(&coef, size: 64)
        AccelerateDCT.idct2D(&coef, size: 64)
        var meanErr: Float = 0, maxErr: Float = 0
        for i in 0..<4096 {
            let e = abs(coef[i] - patch[i])
            meanErr += e
        ...
        meanErr /= 4096
        XCTAssertLessThan(meanErr, 0.03,
            "DCT64 block round-trip mean error \(meanErr)")
        XCTAssertLessThan(maxErr, 0.20,
            "DCT64 block round-trip max error \(maxErr)")
    }

    /// DCT16×32 encoder DSP foundation. `forwardDCT16x32Block`
    /// round-trips a 16-row × 32-col patch (32w × 16h pixel,
    /// DCT16×32 coef layout matches the IDCT output directly)
    /// through the decoder's primitives within bounded error.
    func testVarDCTEncoder_ForwardDCT16x32Block_RoundTrip() throws {
        var patch = [Float](repeating: 0, count: 512)
        for r in 0..<16 {
            for c in 0..<32 {
                patch[r * 32 + c] = 0.12 + Float(r + c) * 0.004
            }
        }
        let qw = try QuantWeights.getQuantWeights(
            rows: 16, cols: 32, bands: DefaultQuantBands.dct16x32)
        let yWeights = Array(qw[512..<1024])
        let globalScale: UInt32 = 5111
        let qf: Int32 = 5
        let acScale = Float(globalScale) / 65536.0
        let (dc, ac) = VarDCTEncoder.forwardDCT16x32Block(
            patch: patch, quantWeights: yWeights,
            scale: acScale, qf: qf)
        XCTAssertEqual(dc.count, 8)
        for p in [0, 1, 2, 3, 32, 33, 34, 35] {
            XCTAssertEqual(ac[p], 0,
                "LLF position \(p) must be 0 in the AC array")
        }
        // Decoder reconstruction.
        var coef = [Float](repeating: 0, count: 512)
        let llf = LowestFrequenciesFromDC.ord6Block(dc: dc)
        for r in 0..<2 {
            for c in 0..<4 { coef[r * 32 + c] = llf[r * 4 + c] }
        }
        ...
        AccelerateDCT.idct2D(&coef, rows: 16, cols: 32)
        // DCT16×32: pixel = coef layout directly.
        var meanErr: Float = 0, maxErr: Float = 0
        for i in 0..<512 {
            let e = abs(coef[i] - patch[i])
            meanErr += e
        ...
        meanErr /= 512
        XCTAssertLessThan(meanErr, 0.03,
            "DCT16x32 block round-trip mean error \(meanErr)")
        XCTAssertLessThan(maxErr, 0.12,
            "DCT16x32 block round-trip max error \(maxErr)")
    }

    /// DCT32×16 encoder DSP foundation. `forwardDCT32x16Block`
    /// transposes the 32-row × 16-col patch into the 16-row × 32-col
    /// coef layout; the round-trip through the decoder's primitives
    /// (with the transpose back to pixels) stays within bound.
    func testVarDCTEncoder_ForwardDCT32x16Block_RoundTrip() throws {
        var patch = [Float](repeating: 0, count: 512)
        for r in 0..<32 {
            for c in 0..<16 {
                patch[r * 16 + c] = 0.12 + Float(r + c) * 0.004
            }
        }
        let qw = try QuantWeights.getQuantWeights(
            rows: 16, cols: 32, bands: DefaultQuantBands.dct16x32)
        ...
        let qf: Int32 = 5
        let acScale = Float(globalScale) / 65536.0
        let (dc, ac) = VarDCTEncoder.forwardDCT32x16Block(
            patch: patch, quantWeights: yWeights,
            scale: acScale, qf: qf)
        XCTAssertEqual(dc.count, 8)
        // Decoder reconstruction.
        ...
        AccelerateDCT.idct2D(&coef, rows: 16, cols: 32)
        // DCT32×16: pixel layout is the transpose of coef —
        // `pixel[r][c] = coef[c * 32 + r]` for r in 0..32, c in 0..16.
        var meanErr: Float = 0, maxErr: Float = 0
        for r in 0..<32 {
            for c in 0..<16 {
                let e = abs(coef[c * 32 + r] - patch[r * 16 + c])
                meanErr += e
                maxErr = max(maxErr, e)
            }
        }
        meanErr /= 512
        XCTAssertLessThan(meanErr, 0.03,
            "DCT32x16 block round-trip mean error \(meanErr)")
        XCTAssertLessThan(maxErr, 0.12,
            "DCT32x16 block round-trip max error \(maxErr)")
    }
    ...
    /// Multi-section `VarDCTBitstreamWriter`. A 384×384 frame spans a
    /// 2×2 grid of 256-px AC groups, so the codestream is written as
    /// LfGlobal + DC-group + HfGlobal + 4 AC-group TOC sections. Each
    /// AC group's nnz-prediction planes reset at the group boundary.
    /// Verified end-to-end with our decoder and libjxl `djxl`.
    func testVarDCTBitstreamWriter_MultiSection() throws {
        let dim = 384
        var frame = ImageFrame(width: dim, height: dim, channels: 3)
        for y in 0..<dim {
            for x in 0..<dim {
                let i = (y * dim + x) * 3
                frame.data[i + 0] = UInt8((40 + x) & 0xff)
                frame.data[i + 1] = UInt8((50 + y) & 0xff)
                frame.data[i + 2] = UInt8((70 + (x + y) / 2) & 0xff)
            }
        }
        func meanError(_ pix: Data, binOffset: Int) -> Double {
            var s = 0
        ...
        XCTAssertLessThan(oursErr, 4.0,
            "multi-section round-trip mean error (our decoder) "
            + "\(oursErr)")
        ...
        let tmp = NSTemporaryDirectory()
        let jxlPath = tmp + "vdt_multi.jxl"
        let outPath = tmp + "vdt_multi_dj.ppm"
        try codestream.write(to: URL(fileURLWithPath: jxlPath))
        let p = Process()
        p.launchPath = djxl
        p.arguments = [jxlPath, outPath]
        ...
        try p.run(); p.waitUntilExit()
        XCTAssertEqual(p.terminationStatus, 0,
            "djxl rejected our multi-section VarDCT codestream")
        let djData = try Data(contentsOf: URL(fileURLWithPath: outPath))
        var binStart = 0, nl = 0
        for (i, b) in djData.enumerated() {
            if b == 0x0A { nl += 1; if nl == 3 { binStart = i + 1; break } }
    ...
    /// Multi-section RGBA `VarDCTBitstreamWriter`. A 384×384 RGBA
    /// frame spans a 2×2 grid of 256-px groups, so the alpha extra
    /// channel exceeds one modular group and is **deferred per AC
    /// group** — each AC-group TOC section carries its VarDCT tokens
    /// then a modular GroupHeader + that group's alpha sub-rect. The
    /// alpha must still be byte-exact; RGB stays lossy within
    /// `mean < 4`. Verified with our decoder and libjxl `djxl`.
    func testVarDCTBitstreamWriter_RGBA_MultiSection() throws {
        let dim = 384
        var frame = ImageFrame(
            width: dim, height: dim, channels: 4, alphaChannels: 1)
        for y in 0..<dim {
            for x in 0..<dim {
                ...
                frame.data[i + 1] = UInt8((50 + y) & 0xff)
                frame.data[i + 2] = UInt8((70 + (x + y) / 2) & 0xff)
                frame.data[i + 3] = UInt8((x * 2 + y * 3) & 0xff)
            }
        }
        let codestream = try VarDCTBitstreamWriter.encode(frame: frame)
        ...
        XCTAssertEqual(decoded.width, dim)
        XCTAssertEqual(decoded.channels, 4,
            "multi-section RGBA should round-trip 4 channels")
        var rgbErr = 0
        for i in 0..<(dim * dim) {
            for c in 0..<3 {
                rgbErr += abs(Int(decoded.data[i * 4 + c])
                              - Int(frame.data[i * 4 + c]))
            }
            XCTAssertEqual(decoded.data[i * 4 + 3], frame.data[i * 4 + 3],
                "multi-section alpha must be lossless at pixel \(i)")
        }
        let oursMean = Double(rgbErr) / Double(dim * dim * 3)
        XCTAssertLessThan(oursMean, 4.0,
            "multi-section RGBA RGB round-trip mean error "
            + "(our decoder) \(oursMean)")
        ...
        let tmp = NSTemporaryDirectory()
        let jxlPath = tmp + "vdt_rgba_multi.jxl"
        let outPath = tmp + "vdt_rgba_multi_dj.pam"
        try codestream.write(to: URL(fileURLWithPath: jxlPath))
        let p = Process()
        p.launchPath = djxl
        p.arguments = [jxlPath, outPath]
        ...
        let djErr = String(
            ...
            encoding: .utf8) ?? ""
        XCTAssertEqual(p.terminationStatus, 0,
            "djxl rejected our multi-section RGBA codestream; "
            + "stderr: \(djErr)")
        let pam = try Data(contentsOf: URL(fileURLWithPath: outPath))
        guard let endhdr = pam.range(
            of: "ENDHDR\n".data(using: .utf8)!) else {
        ...
        for i in 0..<(dim * dim) {
            ...
            XCTAssertEqual(pam[pixelStart + i * 4 + 3],
                frame.data[i * 4 + 3],
                "djxl multi-section alpha must be lossless at pixel \(i)")
        }
        let djMean = Double(djRGB) / Double(dim * dim * 3)
        XCTAssertLessThan(djMean, 4.0,
            "multi-section RGBA RGB round-trip mean error (djxl) "
            + "\(djMean)")
    }
