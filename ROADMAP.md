...
| Lossless JPEG-1 → JXL transcoding (forward) | 🎉 **shipped** v0.12.0fk for all-zero-coefficient bridge fixtures — `JXLEncoder.encodeFromJPEGCoefficients(_:)` produces bytes that `djxl` decodes to correct pixels (verified end-to-end against libjxl 0.11.2). The single-section bit-stream continuity fix (matches libjxl `is_small_image` short-circuit) was the closing piece after the codestream-envelope work in v0.12.0v–ff and the section-content fixes in v0.12.0fh/fi. **Next bite**: real-content JPEG fixture + `JPEGDecoder.decode(jpgBytes)` pixel-parity test to verify non-zero-coefficient paths also work. |
