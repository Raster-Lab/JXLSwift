...
| **v0.8.0 (in progress)** — multi-AC-strategy per-block decode | 🚧 — **Bites a–f shipped** with 332 tests passing: (a) `ACStrategyImage` assembly from ACMeta channel 2 with raster-walk + first-block expansion; (b) AC decode loop iterates first-blocks only, allocating `coveredBlocks * 64` per channel; (c) `CoeffOrders.naturalCoeffOrder(for:)` ports libjxl `CoeffOrderAndLut` for all 13 ords (DCT8x8 byte-equal to hand-coded table, all strategies verified by `testVarDCT_NaturalCoeffOrder_Invariants`); (d) DCT16x16 IDCT pass — `LowestFrequenciesFromDC.dct16x16` reinterprets the 2×2 DC values as LLF coefs (forward 2×2 scaled DCT + `<2,16>` resample factors), `DefaultQuantBands.dct16x16/32x32/8x16` ship the bands, and an overlay pass post-dequant applies LLF + AC-dequant + bridge×16 + IDCT16 + plane placement; (e) 2D `nzPlane` propagates multi-block first-block nzeros to ALL covered cells (matching libjxl `dec_group.cc:488-493`'s `(nzeros + covered_blocks - 1) >> log2_covered_blocks` ceiling-div stamp), and AC channel iteration corrected to libjxl's storage `{1, 0, 2}` (X, Y, B) — fixes a latent Y/X swap at `bctx.context` masked in single-cluster fixtures because all clusters routed to the same ANS distribution; (f) Lehmer permutation decode — `CoeffOrders.decodeLehmerCode` ports libjxl `lehmer_code.h::DecodeLehmerCode` (Fenwick order-statistics tree), `CoeffOrders.decodePermutations` reads + composes with natural order, AC decode loop routes per-channel orders via `coeffOrdersPerPass[pass][ord][storageC]` (verified by `testVarDCT_DecodeLehmerCode_RoundTrip`). **Pending**: residual d=0.5 SWEEP frontier still hits a token-stream out-of-bounds at block (6,4) — Lehmer + iteration fix advanced 2 bits but rANS still diverges, suggesting a context-computation subtlety; DCT32x32, DCT16x8/8x16, DCT4x8/8x4, AFV per-strategy IDCT. |
