...
| **v0.8.0 (in progress)** — multi-AC-strategy per-block decode | 🚧 — **Bites a–i shipped** with 332 tests passing and SWEEP fixtures d=1.0 + d=2.0 fully decoded: (a) `ACStrategyImage` assembly from ACMeta channel 2 with raster-walk + first-block expansion; (b) AC decode loop iterates first-blocks only, allocating `coveredBlocks * 64` per channel; (c) `CoeffOrders.naturalCoeffOrder(for:)` ports libjxl `CoeffOrderAndLut` for all 13 ords; (d) DCT16x16 IDCT — `LowestFrequenciesFromDC.dct16x16` 2×2 forward DCT + `<2,16>` resample, `DefaultQuantBands.dct16x16` bands, overlay pass with bridge×16 + IDCT16 + plane placement; (e) 2D `nzPlane` propagates multi-block nzeros to all covered cells, AC channel iteration corrected to libjxl's storage `{1, 0, 2}` (X, Y, B); (f) Lehmer permutation decode — `CoeffOrders.decodeLehmerCode` (Fenwick OST tree) + `decodePermutations` + per-channel order routing in AC decode (`testVarDCT_DecodeLehmerCode_RoundTrip`); (h) DCT8x16/DCT16x8 IDCT (libjxl ord 4) — `LowestFrequenciesFromDC.ord4Pair` 1-D forward DCT-2 + `<2,16>` resample, `DefaultQuantBands.dct8x16` bands, overlay with bridge×√128 + 16×8 IDCT + transposed-or-direct placement (DCT16x8 transposes IDCT output to get 8w × 16h pixels; DCT8x16 places direct as 16w × 8h); (i) DCT32x16/DCT16x32 IDCT (libjxl ord 6) — `LowestFrequenciesFromDC.ord6Block` 2-D scaled DCT (1-D DCT-2 along rows + 1-D DCT-4 along cols) + `<4,32>` × `<2,16>` resample, `DefaultQuantBands.dct16x32` bands, overlay with bridge×√512 + 32×16 IDCT + transposed-or-direct placement. **SWEEP status**: d=1.0 ✅ DECODED, d=2.0 ✅ DECODED, d=5.0/d=10 advance past VarDCT into EPF restoration layer (frontier now `EPF unsupportedNonZeroSharpness`), d=0.5 still hits a token-stream out-of-bounds at DCT16x16 block (6,4) — a deeper bug independent of the Lehmer/iteration fixes (single-cluster fixture so context routing is moot). **Pending**: d=0.5 deep debug, EPF non-zero sharpness, DCT32x32, DCT4x8/8x4, AFV, DCT64+/larger strategies. |
