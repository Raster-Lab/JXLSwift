...
| Lossless JPEG-1 → JXL transcoding (forward) | 🎉 **shipped** v0.12.0fr — `JXLEncoder.encodeFromJPEGCoefficients(_:)` produces bytes that `djxl` decodes to pixels matching `JPEGDecoder.decode(jpgBytes)` byte-for-byte within ±2 JPEG-decode rounding tolerance (same tolerance `cjxl --lossless_jpeg=1 + djxl` exhibits vs `djpeg`). Covers any 4:4:4 / 8-bit / 1-or-3-component / baseline-DCT JPEG. The closing-the-loop trajectory: v0.12.0fo (DC scale inversion) `max=209→139`, v0.12.0fq (AC coefficient transpose at adapter) `139→82`, v0.12.0fr (`base_correlation_b=0` for non-XYB frames) `82→2`. **Next bite**: lift the 4:4:4-only restriction (4:2:0 / 4:2:2 chroma subsampling support). |
