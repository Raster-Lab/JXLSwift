...
| Lossless JPEG-1 → JXL transcoding | ⏳ — block decoder + dequantiser produce quantised coefficients per component; needs IDCT + YCbCr→RGB pixel-reconstruction OR direct JXL-frame bridge (JXL VarDCT can swallow JPEG-quantised coefficients without IDCT — that's the route libjxl takes). |
