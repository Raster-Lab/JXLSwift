...
| Bitwise-identical JXL → JPEG-1 reconstruction | 🎉🎉 **shipped for the common-case JPEG** v0.12.0ge. `JXLToJPEGAdapter.reconstruct(coefficients:jbrd:colorTransform:)` walks `jbrd.markerOrder` and emits a JPEG that matches the source byte-for-byte. Verified end-to-end by `testEndToEnd_ByteIdenticalReconstruct_RealCjxlPayload` on a real cjpeg fixture + cjxl-emitted jbrd. Pending: Brotli compressed-stream support (for JPEGs with large EXIF/XMP/ICC); canonical kICC/kExif/kXMP marker templates; progressive scan support. |
