...
| Lossless JPEG-1 → JXL transcoding | ⏳ — JPEG side is **complete** (raw bytes → dequantised coefficients verified on real fixtures). Remaining work is the JXL VarDCT coefficient bridge (libjxl's transcode shortcut — feed JPEG coefficients into a JXL frame without IDCT), or alternatively an IDCT + YCbCr→RGB path for the pixel-reconstruction route. |
