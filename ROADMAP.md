...
| Per-pixel modular decode driver | §C.7 | ✅ structural with WP | `decodeModularChannel(width:height:staticChannel:groupId:tree:stream:from:wpHeader:)` runs the row-major per-pixel loop: read 6 neighbours → compute property 15 from the live `WeightedPredictor` + 15 standard properties → walk the MA-tree → read a residual token from the post-tree `TokenStreamReader` at `context = leaf.leafId` → `ZigZag.unpack × leaf.multiplier + leaf.predictorOffset + applyLibjxlPredictor(leaf.rawPredictor, neighbourhood, wpResult)` → fold the actual decoded pixel into the WP state. Cross-validation against cjxl drives all 1024 pixels of a 32×32 channel through this pipeline (13 layers deep) including predictor 6 / property 15 paths; decoded values bounded inside Int32. **Remaining for byte-equality with djxl**: multi-channel iteration + Modular Transform application (RCT inverse, Squeeze inverse). |
