...
| **v0.9.0 (in progress)** — pixel byte-equality + AFV + family-API-parity | ⏳ — Pixel byte-equality residual narrowed to a single quantified factor (2.286×) reproducible in `scripts/diagnostics/libjxl_reference_idct.cc`. Closing it requires a side-by-side libjxl trace (1–6 hour bite). AFV foundation primitive (`AFV.transformToPixels`) shipped. **Family-API-parity audit + Phases A/B/C alignment with J2KSwift complete**: shared `CompressionFamily` Swift package, `JXLEncoder`/`JXLDecoder` are now `struct: Sendable`, async + progress-callback overloads, `jxl` CLI canonical name, real `compare`/`completions` subcommands. Encoder foundation primitives shipped (`Gaborish.applyInverse5x5`, `ACQuantize.quantizeBlock`, AFV overlay). 366 tests passing, 0 failures. Detail: [Documentation/STATUS-2026-05.md](Documentation/STATUS-2026-05.md), [Documentation/v0.9.0-pixel-accuracy-investigation.md](Documentation/v0.9.0-pixel-accuracy-investigation.md), [Documentation/FAMILY-API-PARITY.md](Documentation/FAMILY-API-PARITY.md). |
