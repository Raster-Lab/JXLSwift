...
Spec sections from ISO/IEC 18181-1 and ISO/IEC 18181-2. Each row is verified by a test (where ✅) or a milestone target (where ⏳). The current test suite is **574 passing, 6 skipped, 0 failing** (release config, ~38 s on Apple Silicon, as of v0.12.0fk 2026-05-27).
...
| ANS distribution serialisation | §C.6.3.2 | ✅ simple + flat + spec **complex** (full) | constant (1-symbol), simple (1–4 symbols with predefined splits), flat (uniform), and the spec-compliant **complex** mode in `SpecANSDistribution` (read `readComplex` + write `writeComplex`): a `shift` field, per-position log-counts via the hard-coded 7-bit `log_counts` lookup table, RLE for repeated log-counts on read, then per-position refinement bits — quantised onto the representable grid and `djxl`-decodable. This is the path real `>8`-cluster context-map histograms route through, so it is `djxl`-byte-verified indirectly and pinned by `testSpecANSDistribution_WriterHandlesComplex`. *(A separate `ANSDistributionFormat.encodeFull` — a u(13)-per-symbol layout that is **not** spec-compliant — survives only inside the project-internal M0 `MinimalLosslessCodec` placeholder, which never emits a real JXL file.)* |
...
| Histogram clustering | §C.6.4 | ✅ both paths; layout aligned with libjxl `DecodeContextMap` | simple path covering num_clusters ∈ {1, 2, 4, 8} via `bits_per_entry` u(2), **plus the full entropy-coded path** (`writeFullPath` / `readFullPath`, with inverse-MTF on the read side) for wider maps. `write(to:)` emits whichever encoding is smaller. num_clusters is derived from `max(map) + 1` exactly as libjxl does — no leading u(8) prefix. The full path is `djxl`-byte-verified for 16/18/26-cluster maps (JPEG-bridge round-trip) and pinned by a standalone E5 test. `.fullPathNotImplemented` is now thrown only when a map is genuinely unencodable by either path. |
...
| Color correlation + inverse OpsinXYB + sRGB OETF + 8-bit RGB (Bite 4) | ✅ — coefficient-level CfL `X' = X + x_cc_mul·Y`, `B' = B + b_cc_mul·Y`, with `x_cc_mul`/`b_cc_mul` looked up **per 64×64 colour tile** (`acCFLMul`, v0.10.0o) — earlier code used a single tile-(0,0) slope, only valid for frames ≤ 64 px; piped through `OpsinXYB.inverse` → linear RGB → IEC 61966-2-1 sRGB OETF → 8-bit clamped output |
...
| **v0.10.0i** — 🎉 VarDCT pixel byte-equality | ✅ — instrumented-libjxl side-by-side trace closed the v0.9.0 byte-equality goal. Seven decoder bugs fixed (AC channel swap, spurious ×64 on LIBRARY quant matrices, `ComputeScaledIDCT` transpose, wrong LLF resample scales, LLF `ReinterpretingDCT` transpose, `scaledDCT4` √2 scaling, `AdjustQuantBias` per-channel `\|q\|==1` bias). cjxl-d=0.5/d=1.0 SWEEP byte-exact; DCT8x8/16/32/64 fixtures byte-exact. The prior "2.286× factor" was a red herring caused by the channel swap. d=2/5/10 SWEEP retains a small B-channel residual (max ~12–14, mean < 0.7). Detail: CHANGELOG `v0.10.0i`. |
| **v0.10.0j** — AFV confirmed + IDENTITY transform | ✅ — re-measuring AFV after `v0.10.0i` shows AFV-using fixtures at `max=(R=1..3)` vs `djxl` (the v0.10.0g "catastrophic R=156" was the global bugs, now fixed — no AFV-specific change needed). IDENTITY ("hornuss", AC strategy 1) transform ported (`IdentityTransform.swift` + `getIdentityQuantWeights`); the `antiDiag` probe fixture decodes byte-exact. Remaining unimplemented AC strategies: DCT2X2 / DCT4X4 / DCT4X8 / DCT8X4 / DCT32X8 / DCT8X32. Detail: CHANGELOG `v0.10.0j`. |
...
| EPF0 — 7×7 plus-with-diagonals (12-neighbour) kernel | §C.9.2 | ✅ — `EPF.applyEPF0(...)` ports libjxl `render_pipeline/stage_epf.cc::EPF0Stage` (12 neighbour offsets forming a 5×5 plus, 3×3-plus SAD shape per neighbour, `pass0SigmaScale × 1.65` sigma scale, `borderSadMul` on edge pixels). `EPF.applyAllStages` dispatches it when `epf_iters >= 3`. The existing SWEEP corpus uses default `epf_iters = 2` (so only EPF1 + EPF2 fire); a real `epf_iters = 3` fixture for the pin-down byte-equality test is the open follow-up. |
...
| JPEG DQT (quantisation-table) payload parser (`JPEGQuantTable`) | ✅ — `Sources/JXLSwift/JPEG/JPEGQuantTable.swift`. Parses one or more tables per DQT segment with 8-bit or 16-bit precision per ITU-T T.81 §B.2.4.1; 64 zig-zag-ordered values returned as `[UInt16]`. `JPEGStructure.quantTables(in:)` walks the whole file. 6 tests covering 8/16-bit single-table, multi-table-in-one-segment, truncation + invalid-precision-nibble rejection, fixture + sips-JPEG round-trip. `jxl info` prints a one-line `Quant DC:` summary. |
| JPEG DHT (Huffman-table) payload parser (`JPEGHuffmanTable`) | ✅ — `Sources/JXLSwift/JPEG/JPEGHuffmanTable.swift`. Parses one or more tables per DHT segment per ITU-T T.81 §B.2.4.2: `JPEGHuffmanClass` (DC/AC), 4-bit table ID, 16-element bit-length-counts array `Li`, then sum(Li) symbol values in canonical Huffman code order. `JPEGStructure.huffmanTables(in:)` walks the whole file. 7 tests covering DC/AC single-table, multi-table-in-one-segment, truncation + excessive symbol count rejection, fixture + sips-JPEG round-trip. Runtime code-table construction (§C.2 algorithm) is the next bite. |
| JPEG SOFn per-component records + SOS scan header (`JPEGFrameComponent`, `JPEGScanHeader`) | ✅ — `Sources/JXLSwift/JPEG/JPEGScanHeader.swift`. Per-component sampling factors (`Hi`/`Vi`) + quant-table binding (`Tqi`) for the SOFn payload; per-scan component → DC/AC Huffman table binding for SOS, plus spectral-selection + successive-approximation fields for progressive scans. `JPEGStructure.frameComponents(in:)` and `JPEGStructure.scanHeaders(in:)` walk the whole file. 5 tests covering fixture + sips-JPEG paths and truncated-payload rejection. **Closes the structural parsing of a JPEG file** — every non-entropy byte from SOI to EOI now has a typed Swift representation. |
| JPEG canonical Huffman codebook builder (`JPEGHuffmanCodebook`, §C.2) | ✅ — `Sources/JXLSwift/JPEG/JPEGHuffmanCodebook.swift`. `JPEGHuffmanTable.buildCodebook()` ports the §C.2 Figure F.15 algorithm (HUFFSIZE / HUFFCODE then mincode / maxcode / valoffset). `decodeSymbol(nextBit:huffvals:)` is the §C.2 Figure F.16 decode loop. 5 tests: 2-symbol minimal, hand-derived `[0,3,1,…]` codes (00/01/10/110), full encode-then-decode round-trip, sips-JPEG codebook build with maxcode-chain monotonicity assertion, graceful-EOF. |
| JPEG MSB-first bit reader (`JPEGBitReader`) — byte-unstuffing + RST-aware | ✅ — `Sources/JXLSwift/JPEG/JPEGBitReader.swift`. MSB-first per §F.2.2.5; transparently consumes `0xFF 0x00` stuffing per §F.1.2.3; surfaces `0xFF D0..D7` RST markers via `markerSeen` (for DC-differential reset) while silently skipping them; throws `.sawMarker` on a non-RST marker so the caller can re-anchor. 6 tests cover MSB ordering, cross-byte reads, stuffing transparency, RST skip with markerSeen, non-RST → throw + cursor preservation, truncation. |
| JPEG DC + AC coefficient block decoder (`JPEGBlockDecoder`, `JPEGCoefficientBlock`, `JPEGZigZag`) | ✅ — `Sources/JXLSwift/JPEG/JPEGBlockDecoder.swift`. Reads one 8×8 quantised DCT block in natural order. DC path: Huffman → size byte → EXTEND (§F.2.2.1 Figure F.12) → add to per-component `JPEGDCPredictor`. AC path: zig-zag walk over 63 positions with EOB / ZRL / RLE token handling and §F.2.2.1 EXTEND on each magnitude. 7 tests: all-zero, DC=+7, DC=−3, multi-block DC differential, ZRL+value with zig-zag mapping check (pos 17 → natural index 19), truncated propagation, Figure A.6 zig-zag table sanity. |
| JPEG dequantiser (`JPEGDequantiser`) | ✅ — `Sources/JXLSwift/JPEG/JPEGDequantiser.swift`. Multiplies each natural-order coefficient by its zig-zag-indexed quant factor (`block[zigzag[k]] *= Q.zigZagValues[k]`). In-place + functional flavours. 4 tests: identity table, constant multiplier, zig-zag mapping correctness (catches transposition bugs), in-place ≡ functional. |
| JPEG baseline-sequential scan decoder (`JPEGScanDecoder`, `JPEGComponentBlocks`, `JPEGHuffmanCodebookMap`) | ✅ — `Sources/JXLSwift/JPEG/JPEGScanDecoder.swift`. MCU iteration with full `H_i`/`V_i` sampling-factor support; per-component DC predictors; restart-interval handling (reset predictors + byte-align reader every `Ri` MCUs). Rejects progressive scans with clear error. 5 tests: synthetic single-block, synthetic 16×16 grayscale 2×2 MCU walk with DC accumulation, progressive rejection, unknown-component rejection, AND **`testJPEGScanDecoder_RealSIPSFixture` — end-to-end raw JPEG bytes → dequantised DCT coefficients on a sips-emitted 16×16 RGB JPEG**, asserting 3 components with non-zero DCs and `|dequantised| ≥ |quantised|`. |
| JPEG IDCT (`JPEGIDCT`) | ✅ — `Sources/JXLSwift/JPEG/JPEGIDCT.swift`. Reuses `DCT2D.inverse(_:size:8)` (mathematically identical normalisation to JPEG §A.3.3). §A.3.1 level shift + clamp on top. 5 tests including DC-only flat reconstruction and sips real-fixture sample validity. |
...
| Lossless JPEG-1 → JXL transcoding (forward) | 🎉 **shipped** for 4:4:4 single + multi-block (v0.12.0fr / fx) and 4:2:0 / 4:2:2 (v0.12.0ft). Pixel-equivalent within ±2 JPEG-decode rounding tolerance on 4:4:4 (any size, any sampling shape in `{H1V1, H2V1, H1V2, H2V2}` for Y with H1V1 chroma). 4:2:0 bridge output is byte-identical to cjxl's reference bridge (verified manually). Closing trajectory: v0.12.0fr (4:4:4 single-block pixel parity), v0.12.0ft (subsampling structural lift), v0.12.0fx (multi-block DC predictor clamp removal — the `lo: 0, hi: 127` clamp on the gradient predictor was truncating any DC value > 127, breaking blocks beyond `(0, 0)`). |
| Coefficient-identical JXL → JPEG-1 reconstruction | 🎉 **shipped** v0.12.0g1–g5. `JXLToJPEGAdapter.reconstructMinimal(...)` produces a structurally valid baseline JPEG from JXL coefficient planes + supplied Huffman + quant tables. |
| Bitwise-identical JXL → JPEG-1 reconstruction | 🎉🎉🎉 **shipped for the common-case + EXIF JPEG** v0.12.0ge→gn. `JXLToJPEGAdapter.reconstruct(coefficients:jbrd:colorTransform:)` walks `jbrd.markerOrder` and emits a JPEG that matches the source byte-for-byte. CLI: `jxl transcode --mode reverse --source orig.jpg in.jxl out.jpg`. Verified end-to-end on cjpeg fixtures (with and without EXIF metadata). Pending: Brotli static dictionary (for streams with larger metadata that reference the dictionary); NBLTYPES>1 / NTREES>1 streams; canonical kICC marker template (libraries done — body source pending); progressive scan support. |
| `jbrd` box (JPEG bitstream reconstruction data) handling | ✅ shipped v0.12.0g7–g9 + gd. Full Bundle reader + writer (`JBRDBoxReader.read` + `JBRDBoxWriter.write`) round-trips real cjxl payloads. `JBRDBox.distributeBrotliPayload` fills app/com/intermarker/tail bytes from the decoded Brotli stream. |
...
2. **Lossless completeness — v0.14.0.** Each deferred item, resolved with an explicit decision:
   - **E5 >8-cluster context-map `djxl` compliance — ✅ already done.** The full entropy-coded
     ...
     16/18/26-cluster maps via the JPEG bridge; a standalone E5 test pins it. The original deferral
     note was stale. (The lossless *Modular* path still self-caps at 8 leaves — that cap is gated
     by the property-set item below, **not** by the context-map encoding.)
   ...
   - **Reconcile `fillModularProperties` formulas (properties 6,7,8,11,13,14) — documented 1.0
     ...
     reverse-engineered empirically through `djxl` — research-grade, high desync risk, and a prior
