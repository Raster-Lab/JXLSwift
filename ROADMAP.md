...
| Coefficient-identical JXL → JPEG-1 reconstruction | 🎉 **shipped** v0.12.0g1–g5. `JXLToJPEGAdapter.reconstructMinimal(...)` produces a structurally valid baseline JPEG from JXL coefficient planes + supplied Huffman + quant tables. End-to-end test (`testEndToEnd_ForwardThenReverseBridge_CoefficientsMatch`): real JPEG → forward bridge → reverse bridge → rebuilt JPEG decodes to matching coefficients across all components / blocks; `djpeg` accepts the output. |
| Bitwise-identical JXL → JPEG-1 reconstruction | 🚧 in progress — gated on Brotli decoder + `jbrd` box reader. Brotli scaffold landed v0.12.0fz (bit reader, prefix codes, meta-block header; 14 tests). JBRDBox struct + entry-point scaffolds landed v0.12.0g0. Remaining for byte-identical: Brotli alphabets / context modeling / static dictionary / LZ77 (~4-8 sessions), `JBRDBoxReader.read` field walk, `JXLToJPEGAdapter.reconstruct(coefficients:jbrd:...)` integration. |
| `jbrd` box (JPEG bitstream reconstruction data) handling | 🚧 scaffold shipped v0.12.0g0 (`JPEG/JBRDBox.swift`); Bundle field-by-field walk pending. |
