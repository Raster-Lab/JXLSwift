...
| **v0.8.0 (next)** — multi-AC-strategy per-block decode | ⏳ — cjxl emits non-DCT8 strategies (DCT16, DCT32, DCT4×8, AFV, …) for ≥384px fixtures; ACMeta channel 2 carries `count`-entry "first-block" strategy IDs, with the rest expanding to non-first-block slots. Need: ACStrategyImage assembly from ACMeta, per-strategy `coveredBlocks` / `log2CoveredBlocks` / `kNaturalCoeffOrder` / IDCT shape, and skip-non-first-block iteration in the AC decode loop. Substantial — likely 2 sessions. |
