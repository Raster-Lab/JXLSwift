...
| Lossless JPEG-1 → JXL transcoding (forward) | 🎉 **shipped** v0.12.0fr for 4:4:4 + structural-only v0.12.0ft for 4:2:0/4:2:2. 4:4:4: pixel-equivalent within ±2 JPEG-decode rounding tolerance (covers any 4:4:4 / 8-bit / 1-or-3-component / baseline-DCT JPEG). 4:2:0: bridge accepts subsampled inputs, `djxl` decodes the output (max=31 / mean=18.5 byte-diff vs reference — structurally correct, residual section-content diff is the next bite). Closing trajectory for 4:4:4: v0.12.0fo (DC scale inversion) `max=209→139`, v0.12.0fq (AC coefficient transpose) `139→82`, v0.12.0fr (`base_correlation_b=0`) `82→2`. v0.12.0ft (subsampling lift): adds `blocksPerChannel` per-channel block grid, `generateBridgeACTokens` with hshift/vshift gating per libjxl `enc_entropy_coder.cc:196`. |
